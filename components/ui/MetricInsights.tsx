...
export function MetricInsights({ records }: MetricInsightsProps) {
  ...
  const calculateInsights = () => {
    ...
    if (maxXP > 0) {
      insights.push(
        `🎯 Melhor XP/hora: ${Math.round(maxXP).toLocaleString()}/h`
      );
      insights.push(
        `📊 XP/hora médio: ${Math.round(avgXP).toLocaleString()}/h`
      );
      if (maxXP > minXP * 1.5) {
        insights.push(
          `⚡ Diferença significativa entre sessões: ${Math.round(
            ((maxXP - minXP) / minXP) * 100
          )}%`
        );
      }
    }
    ...
    const weaponBuilds = records
      .filter(
        (r) => r.data.weaponDetail?.id && r.data.weaponDetail?.proficiencies
      )
      .map((r, index) => {
        // Usar XP Gain limpo
        ...
        return {
          ...
          profit: cleanProfit,
          hasProficiencies:
            r.data.weaponDetail?.proficiencies &&
            Object.keys(r.data.weaponDetail.proficiencies).length > 0,
        };
      })
    ...
    if (weaponBuilds.length > 1) {
      ...
      if (bestXpBuild.xpGain > 0) {
        insights.push(
          `🎯 Melhor árvore para XP: Registro ${bestXpBuild.recordIndex} - ${
            bestXpBuild.weaponName
          } (${bestXpBuild.xpGain.toLocaleString()} XP)`
        );
      }

      if (bestProfitBuild.profit > 0) {
        insights.push(
          `💰 Árvore mais lucrativa: Registro ${
            bestProfitBuild.recordIndex
          } - ${
            bestProfitBuild.weaponName
          } (+${bestProfitBuild.profit.toLocaleString()} gp)`
        );
      }

      // Verificar se há empate ou diferenças significativas
      const xpDifference = Math.abs(
        bestXpBuild.xpGain - Math.min(...weaponBuilds.map((b) => b.xpGain))
      );
      if (xpDifference < bestXpBuild.xpGain * 0.1) {
        insights.push(
          `📊 XP similar entre builds - diferenças nas habilidades têm pouco impacto`
        );
      }
    } else {
      ...
      insights.push(
        `💰 Sessões lucrativas: ${profitableSessions}/${records.length}`
      );
      if (totalProfit > 0) {
        insights.push(
          `📈 Lucro total combinado: +${totalProfit.toLocaleString()} gp`
        );
      } else if (totalProfit < 0) {
        insights.push(
          `📉 Prejuízo total combinado: ${totalProfit.toLocaleString()} gp`
        );
      }
    }

    // Armas insights
    const weaponSessions = records.filter(
      (r) => r.data.weaponDetail?.id
    ).length;
    if (weaponSessions > 0) {
      insights.push(
        `⚔️ Sessões com arma registrada: ${weaponSessions}/${records.length}`
      );
      ...
      if (weaponTypes.size > 1) {
        insights.push(
          `🎪 ${weaponTypes.size} tipos diferentes de armas usadas`
        );
      }
    }
    ...
    const durations = records
      .map((r) => r.data["Session length"] || "")
      .filter(Boolean);
    if (durations.length > 1) {
      const shortSessions = durations.filter(
        (d) => d.includes("m") && !d.includes("h")
      ).length;
      const longSessions = durations.filter((d) => d.includes("h")).length;

      if (shortSessions > 0 && longSessions > 0) {
        insights.push(
          `⏱️ Mix de sessões: ${shortSessions} curtas, ${longSessions} longas`
        );
      }
    }
  ...
  const generateRecommendations = () => {
    ...
    if (maxXP > minXP * 2) {
      recommendations.push(
        "📈 Grande variação no XP/hora - analise o que torna as melhores sessões mais eficientes"
      );
    }
    ...
    if (lossCount > records.length / 2) {
      recommendations.push(
        "💸 Muitas sessões com prejuízo - considere spots mais lucrativos ou ajuste supplies"
      );
    }
    ...
    if (weaponCount < records.length) {
      recommendations.push(
        "⚔️ Registre informações da arma para análises mais precisas"
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        "✨ Dados consistentes! Continue monitorando para identificar padrões"
      );
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200">
        <Typography
          variant="lead"
          className="font-semibold text-blue-900 dark:text-blue-900 mb-4"
        >
      ...
      <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
        <Typography
          variant="lead"
          className="font-semibold text-green-900 dark:text-green-900 mb-4"
        >
          💡 Recomendações
        </Typography>
        <div className="space-y-2">
